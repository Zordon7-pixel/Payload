  ...
  return (
    <>
      <button onClick={() => setOpen(true)}
        className="fixed bottom-20 right-5 z-40 flex items-center gap-2 bg-amber-500 hover:bg-amber-400 text-white text-sm font-semibold px-4 py-2.5 rounded-full shadow-lg shadow-amber-900/40 transition-all hover:scale-105">
          ...
          <div className="bg-[#1a1d2e] border border-[#2a2d3e] rounded-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
            ...
            {done ? (
              <div className="p-8 text-center">
                ...
                <button onClick={reset} className="text-amber-400 hover:text-amber-300 text-sm underline">Submit more feedback</button>
              </div>
            ) : (
              <>
                <div className="overflow-y-auto flex-1 p-5 space-y-4">
                  ...
                  <div>
                    ...
                    <input value={name} onChange={e => setName(e.target.value)} placeholder="First name is fine"
                      className="w-full bg-[#0f1117] border border-[#2a2d3e] rounded-lg px-3 py-2 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-amber-500" />
                  </div>
                    ...
                    <div key={i} className="bg-[#0f1117] border border-[#2a2d3e] rounded-xl p-4 space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <select value={item.category} onChange={e => setItem(i, 'category', e.target.value)}
                            className="w-full bg-[#1a1d2e] border border-[#2a2d3e] rounded-lg px-2 py-1.5 text-xs text-white focus:outline-none focus:border-amber-500">
                      ...
                      <div>
                        ...
                        <textarea value={item.message} onChange={e => setItem(i, 'message', e.target.value)} rows={3}
                          ...
                          className="w-full bg-[#1a1d2e] border border-[#2a2d3e] rounded-lg px-3 py-2 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-amber-500 resize-none" />
                      </div>
                      ...
                      {(item.category === 'bug' || item.category === 'missing') && (
                        <div>
                          ...
                          <input value={item.expected} onChange={e => setItem(i, 'expected', e.target.value)}
                            ...
                            className="w-full bg-[#1a1d2e] border border-[#2a2d3e] rounded-lg px-3 py-2 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-amber-500" />
                        </div>
                      )}
                  ...
                  <button onClick={addItem}
                    className="w-full flex items-center justify-center gap-2 border border-dashed border-[#2a2d3e] hover:border-amber-500/50 text-slate-500 hover:text-amber-400 rounded-xl py-2.5 text-xs transition-all">
                ...
                <div className="p-5 border-t border-[#2a2d3e] flex-shrink-0">
                  <button onClick={submit} disabled={loading || !items.some(it => it.message.trim())}
                    className="w-full flex items-center justify-center gap-2 bg-amber-500 hover:bg-amber-400 disabled:opacity-40 text-black font-bold rounded-lg py-2.5 text-sm transition-colors">
                    <Send size={14} />
